...
def test_frame_from_device_requires_checksum():
...
def test_frame_from_device_requires_source_frame():
...
def test_verify_checksum_with_matching_value(device_frame_factory):
    frame = Frame(**device_frame_factory())
    assert frame.verify_checksum() is True


def test_verify_checksum_returns_false_with_mismatch(device_frame_factory):
    frame = Frame(**device_frame_factory(checksum="00"))
    assert frame.verify_checksum() is False


def test_is_ping_response_detects_device_versions(device_frame_factory):
    frame = Frame(**device_frame_factory())
    assert frame.is_ping_response() is True


def test_is_ping_response_false_without_version_string(device_frame_factory):
    frame = Frame(**device_frame_factory(source_frame_from_device="ACK 0 PING"))
    assert frame.is_ping_response() is False


def test_is_order_response_with_data(device_frame_factory):
...
def test_is_order_response_without_data(device_frame_factory):
...
def test_has_response_error(device_frame_factory):
