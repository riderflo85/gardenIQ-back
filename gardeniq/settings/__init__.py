...
def get_config_value(
    ...
    casting_error_message_template: str = (
        'Unable to cast "{config_name}": {exception}'
    )
) -> Any:
    ...
    Args:
        ...
        default (Optional[Any], optional): the default value to fallback to. Do not set the default to mark the config as required.
            Defaults to CONFIG_DEFAULT_NOT_SET.
        cast (Optional[Callable[[Any], Any]], optional): a callable function to format the raw value.
            Defaults to None.
        missing_config_message_template (str, optional): the error message to show when no configuration value could be retrieved
            and no default value was provied.
            Defaults to ( 'Unable to fecth "%(config_name)s" configuration, and no default value was provided.' ).
        casting_error_message_template (str, optional): the error message to shwo when trying to cast environment variable.
